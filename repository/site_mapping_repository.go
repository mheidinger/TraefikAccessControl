...
func (rep *SiteMappingRepository) GetBySite(siteID int) (siteMappings []*models.SiteMapping, err error) {
	err = databaseConnection.Where(&models.SiteMapping{SiteID: siteID}).Find(&siteMappings).Error
	return
}
...
func (rep *SiteMappingRepository) Delete(userID, siteID int) (err error) {
	err = databaseConnection.Where(&models.SiteMapping{UserID: userID, SiteID: siteID}).Delete(&models.SiteMapping{}).Error
	return
}
